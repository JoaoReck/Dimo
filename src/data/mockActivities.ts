import { Activity, DayOffset, DayInfo } from '../types';

export const INITIAL_DAYS_INFO: Record<DayOffset, DayInfo> = {
  [-1]: {
    offset: -1,
    dateKey: 'ontem',
    label: 'ONTEM // QUARTA, 23 OUT',
    shortLabel: 'Ontem',
    dateFormatted: '23 de Outubro',
  },
  [0]: {
    offset: 0,
    dateKey: 'hoje',
    label: 'HOJE // QUINTA, 24 OUT',
    shortLabel: 'Hoje',
    dateFormatted: '24 de Outubro',
  },
  [1]: {
    offset: 1,
    dateKey: 'amanha',
    label: 'AMANHÃ // SEXTA, 25 OUT',
    shortLabel: 'Amanhã',
    dateFormatted: '25 de Outubro',
  },
};

export const INITIAL_ACTIVITIES_BY_DAY: Record<DayOffset, Activity[]> = {
  [-1]: [
    {
      id: 'ontem-1',
      title: 'Café & Planejamento Estratégico',
      startTime: '08:00',
      endTime: '08:45',
      duration: '45 min',
      completed: true,
      completedAt: '08:42',
      description: 'Revisão da semana, preparo do café filtrado e organização das metas críticas.',
      category: 'Rotina',
    },
    {
      id: 'ontem-2',
      title: 'Trabalho / Sprint de Implementação',
      startTime: '09:00',
      endTime: '12:00',
      duration: '3h 00min',
      completed: true,
      completedAt: '11:58',
      description: 'Refatoração da camada de dados e estruturação dos componentes reativos.',
      category: 'Foco Profissional',
    },
    {
      id: 'ontem-3',
      title: 'Almoço & Caminhada Regenerativa',
      startTime: '12:00',
      endTime: '13:00',
      duration: '1h 00min',
      completed: true,
      completedAt: '12:55',
      description: 'Pausa nutritiva sem telas. Caminhada de 20 minutos sob luz solar direta.',
      category: 'Recuperação',
    },
    {
      id: 'ontem-4',
      title: 'Treino de Força & Mobilidade',
      startTime: '18:30',
      endTime: '19:45',
      duration: '1h 15min',
      completed: true,
      completedAt: '19:40',
      description: 'Levantamento composto, agachamento búlgaro e liberação miofascial.',
      category: 'Saúde & Físico',
    },
    {
      id: 'ontem-5',
      title: 'Estudo: Design Systems & Tipografia',
      startTime: '20:00',
      endTime: '21:15',
      duration: '1h 15min',
      completed: true,
      completedAt: '21:10',
      description: 'Leitura de documentação sobre escalas ópticas e grid proporcional.',
      category: 'Desenvolvimento',
    },
  ],

  [0]: [
    {
      id: 'hoje-1',
      title: 'Café & Preparação',
      startTime: '08:00',
      endTime: '08:45',
      duration: '45 min',
      completed: true,
      completedAt: '08:44',
      description: 'Alinhamento cognitivo, leitura rápida de diretrizes do sprint e preparo físico inicial.',
      category: 'Rotina',
    },
    {
      id: 'hoje-2',
      title: 'Trabalho / Arquitetura & Código',
      startTime: '09:00',
      endTime: '11:30',
      duration: '2h 30min',
      completed: true,
      completedAt: '11:28',
      description: 'Modelagem do core de física, cálculo de curvas Bezier e refatoração de nós reativos na engine.',
      category: 'Foco Profissional',
    },
    {
      id: 'hoje-3',
      title: 'Almoço & Descanso',
      startTime: '12:00',
      endTime: '13:00',
      duration: '1h 00min',
      completed: true,
      completedAt: '12:50',
      description: 'Desconexão dos monitores. Reabastecimento calórico e caminhada para recuperação cognitiva.',
      category: 'Recuperação',
    },
    {
      id: 'hoje-4',
      title: 'Treino Funcional de Força',
      startTime: '18:30',
      endTime: '19:30',
      duration: '1h 00min',
      completed: false,
      description: 'Sessão de força composta e sprints cardiovasculares. Aceleração da circulação sanguínea.',
      category: 'Saúde & Físico',
    },
    {
      id: 'hoje-5',
      title: 'Estudar: Princípios de Motion & Interface',
      startTime: '20:00',
      endTime: '21:15',
      duration: '1h 15min',
      completed: false,
      description: 'Leitura aprofundada dos algoritmos de amortecimento inercial, cinemática direta e transições com aceleração variável.',
      category: 'Desenvolvimento',
    },
  ],

  [1]: [
    {
      id: 'amanha-1',
      title: 'Café & Respiração Consciente',
      startTime: '08:00',
      endTime: '08:45',
      duration: '45 min',
      completed: false,
      description: 'Rotina matinal, hidratação e definição das três prioridades absolutas do dia.',
      category: 'Rotina',
    },
    {
      id: 'amanha-2',
      title: 'Trabalho / Entrega & Validação',
      startTime: '09:00',
      endTime: '12:00',
      duration: '3h 00min',
      completed: false,
      description: 'Revisão final de integrações, execução da bateria de testes e entrega de versão.',
      category: 'Foco Profissional',
    },
    {
      id: 'amanha-3',
      title: 'Almoço & Descompressão',
      startTime: '12:00',
      endTime: '13:00',
      duration: '1h 00min',
      completed: false,
      description: 'Alimentação equilibrada e pausa mental longe de dispositivos eletrônicos.',
      category: 'Recuperação',
    },
    {
      id: 'amanha-4',
      title: 'Treino Cardio & Resistência',
      startTime: '18:30',
      endTime: '19:30',
      duration: '1h 00min',
      completed: false,
      description: 'Corrida intervalada de alta intensidade e trabalho de flexibilidade articular.',
      category: 'Saúde & Físico',
    },
    {
      id: 'amanha-5',
      title: 'Estudar & Leitura Livre',
      startTime: '20:00',
      endTime: '21:00',
      duration: '1h 00min',
      completed: false,
      description: 'Capítulo sobre modelos mentais e síntese de aprendizados da semana.',
      category: 'Desenvolvimento',
    },
  ],
};
